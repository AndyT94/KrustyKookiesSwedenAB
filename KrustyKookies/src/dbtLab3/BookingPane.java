...
public class BookingPane extends BasicPane {
	...
	class DateSelectionListener implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			fields[MOVIE_NAME].setText(p.movie);
			fields[PERF_DATE].setText(p.date);
			fields[THEATER_NAME].setText(p.theater);
			fields[FREE_SEATS].setText(Integer.toString(p.available_seats));
		}
	}
	...
	class ActionHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			String date = dateList.getSelectedValue();
			/* --- insert own code here --- */
			if (db.makeReservation(movieName, date)) {
				displayMessage("Booked one seat for " + movieName + " at " + date + "!");
				int seats = Integer.parseInt(fields[FREE_SEATS].getText()) - 1;
				fields[FREE_SEATS].setText(Integer.toString(seats));
			} else {
				displayMessage("This performance is fully booked!");
			}
